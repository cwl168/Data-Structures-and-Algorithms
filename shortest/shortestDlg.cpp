...
void CShortestDlg::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if(i<g.vexnum) // 鼠标点击了序号为i的顶点
	{
		...
		else // 鼠标左键单击偶数次（点击终点）
		{
			...
			{
				path(g,d,p,number[0],number[1],pa); // 求最短路径上由起点城市到终点城市沿途所经过的城市，其顶点序号存于数组pa
...
void CShortestDlg::path(MGraph G,DistancMatrix D,PathMatrix P,int i, int j, int pa[])
{//求由序号为i的起点到序号为j的终点最短路径沿途所经过的顶点，pa[]存放依次经过的顶点
  char p[MAX_VERTEX_NUM];//一维数组，存放P[i][j]
  int k, n, m=i;//起点序号赋给m
  int n1=0;
  VRType a;
  for(k=0;k<G.vexnum;k++)//加
	pa[k]=-1; // pa的初值
  if(i!=j)//两顶点不同
  {
    for(k=0; k<G.vexnum; k++)//给p[]赋值
      p[k]=P[i][j][k];
    if(D[i][j]<INFINITY)//有通路
    {
      while(m!=j)//未到终点
      {
        a=INFINITY;//找最小值，设置初值为最大值
        for(k=0; k<G.vexnum; k++)//查找顶点[m]的所有邻接顶点
          if(G.arcs[m][k].adj<a && p[k])
          {//m到k有较短的直接通路，且k在i到j的最短路径上
            a=G.arcs[m][k].adj;//修改a为较小值
            n=k;//较小值顶点序号赋给n
          }
        pa[n1++]=n;//加
        p[n]=false;
